...
<div class="border-b border-gold-200 mb-7 flex gap-1 text-sm font-semibold uppercase tracking-wide">
    @foreach ([
        ...
        @if (auth()->user()->hasPermission($tab['permission']))
            <a href="{{ route($tab['route']) }}"
               class="px-4 py-3 border-b-2 -mb-px {{ $active === $key ? 'border-gold-600 text-gold-700' : 'border-transparent text-ink/50 hover:text-gold-700' }}">
                {{ $tab['label'] }}
            </a>
        @endif
    @endforeach
